...
- JS (TS)
    ...
    type DepartmentFieldsResult = Record<string, string>

    try {
      const response = await $b24.actions.v2.call.make<DepartmentFieldsResult>({
      ...
      if (!response.isSuccess) {
        ...
        console.info('Department fields:', result)
      }
    } catch (error) {
...
- JS (UMD)
    ...
    <script>
      async function getDepartmentFields() {
        try {
      ...
      document.addEventListener('DOMContentLoaded', getDepartmentFields)
    </script>
    ```
