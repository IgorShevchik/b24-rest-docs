...
1. Получить события пользователя с `id` = `1` за период с `2024-06-20` по `2024-08-20`
    ...
    - JS (TS)
        ...
        // Shape of each CalendarEvent returned in result[]
        type CalendarEvent = {
          ID: string
          PARENT_ID: string
          DELETED: string
        ...
        try {
          const response = await $b24.actions.v2.call.make<CalendarEvent[]>({
          ...
          if (!response.isSuccess) {
            ...
            console.info('Calendar events:', result.length, result[0]?.NAME)
          }
        } catch (error) {
    ...
    - JS (UMD)
        ...
        <script>
          ...
          document.addEventListener('DOMContentLoaded', getUserCalendarEvents)
        </script>
        ```
    ...
    - JS (TS)
        ...
        try {
          ...
          if (!response.isSuccess) {
            ...
            const result = response.getData()!.result
            console.info('Company calendar events:', result.length, result[0]?.NAME)
          }
        } catch (error) {
          // Thrown on transport or SDK failures (AjaxError, SdkError, etc.)
          console.error(error)
