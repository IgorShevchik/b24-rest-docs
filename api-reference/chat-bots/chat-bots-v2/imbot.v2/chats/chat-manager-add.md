...
- JS (TS)
    ...
    type ChatManagerAddResult = {
      result: boolean
    }

    try {
      const response = await $b24.actions.v2.call.make<ChatManagerAddResult>({
        method: 'imbot.v2.Chat.Manager.add',
        params: {
          botId: 456,
          dialogId: 'chat5',
          userIds: [4, 5],
      ...
      if (!response.isSuccess) {
        ...
        console.info(result.result) // true if managers were added successfully
      }
    } catch (error) {
...
- JS (UMD)
    ...
    <script>
      async function addChatManagers() {
        try {
      ...
      document.addEventListener('DOMContentLoaded', addChatManagers)
    </script>
    ```
