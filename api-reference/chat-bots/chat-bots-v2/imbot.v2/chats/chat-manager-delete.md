...
- JS (TS)
    ...
    type ChatManagerDeleteResult = {
      result: boolean
    }

    try {
      const response = await $b24.actions.v2.call.make<ChatManagerDeleteResult>({
        method: 'imbot.v2.Chat.Manager.delete',
        params: {
          botId: 456,
          dialogId: 'chat5',
          userIds: [4, 5],
      ...
      if (!response.isSuccess) {
        ...
        console.info('Managers removed:', result.result)
      }
    } catch (error) {
...
- JS (UMD)
    ...
    <script>
      async function deleteChatManagers() {
        try {
      ...
      document.addEventListener('DOMContentLoaded', deleteChatManagers)
    </script>
    ```
