...
- JS (TS)
    ...
    try {
      const response = await $b24.actions.v2.call.make<MessageGetResult>({
        method: 'imbot.v2.Chat.Message.get',
        params: {
          botId: 456,
          messageId: 789,
        },
      ...
      if (!response.isSuccess) {
        ...
        console.info(result.message.id, result.message.text, result.user.name)
      }
    } catch (error) {
...
- JS (UMD)
    ...
    <script>
      ...
      document.addEventListener('DOMContentLoaded', getChatMessage)
    </script>
    ```
