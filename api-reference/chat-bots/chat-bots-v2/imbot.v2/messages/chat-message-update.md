...
- JS (TS)
    ...
    type ChatMessageUpdateResult = {
      result: boolean
    }

    try {
      const response = await $b24.actions.v2.call.make<ChatMessageUpdateResult>({
        method: 'imbot.v2.Chat.Message.update',
        params: {
          botId: 456,
          messageId: 789,
      ...
      if (!response.isSuccess) {
        ...
        console.info('Message updated:', result.result)
      }
    } catch (error) {
...
- JS (UMD)
    ...
    <script>
      ...
      document.addEventListener('DOMContentLoaded', updateChatMessage)
    </script>
    ```
