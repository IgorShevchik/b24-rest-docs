...
- JS (TS)
    ...
    type ReactionAddResult = {
      result: boolean
    }

    try {
      const response = await $b24.actions.v2.call.make<ReactionAddResult>({
        method: 'imbot.v2.Chat.Message.Reaction.add',
        params: {
          botId: 456,
          messageId: 789,
          reaction: 'like',
      ...
      if (!response.isSuccess) {
        ...
        console.info('Reaction added:', result.result)
      }
    } catch (error) {
...
- JS (UMD)
    ...
    <script>
      ...
      document.addEventListener('DOMContentLoaded', addReaction)
    </script>
    ```
