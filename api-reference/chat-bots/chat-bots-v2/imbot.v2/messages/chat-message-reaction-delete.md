...
- JS (TS)
    ...
    try {
      const response = await $b24.actions.v2.call.make<DeleteReactionResult>({
        method: 'imbot.v2.Chat.Message.Reaction.delete',
        params: {
          botId: 456,
          messageId: 789,
          reaction: 'like',
      ...
      if (!response.isSuccess) {
        ...
        console.info('Reaction deleted:', result.result)
      }
    } catch (error) {
...
- JS (UMD)
    ...
    <script>
      async function deleteChatMessageReaction() {
        try {
      ...
      document.addEventListener('DOMContentLoaded', deleteChatMessageReaction)
    </script>
    ```
