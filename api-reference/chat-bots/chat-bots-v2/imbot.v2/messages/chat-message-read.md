...
- JS (TS)
    ...
    try {
      const response = await $b24.actions.v2.call.make<MarkMessagesReadResult>({
        method: 'imbot.v2.Chat.Message.read',
        params: {
          botId: 456,
          dialogId: 'chat5',
          messageId: 789,
      ...
      if (!response.isSuccess) {
        ...
        console.info(result.chatId, result.lastId, result.counter, result.viewedMessages)
      }
    } catch (error) {
...
- JS (UMD)
    ...
    <script>
      ...
      document.addEventListener('DOMContentLoaded', markMessagesRead)
    </script>
    ```
