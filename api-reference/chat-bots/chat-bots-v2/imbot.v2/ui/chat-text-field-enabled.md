...
- JS (TS)
    ...
    type TextFieldEnabledResult = {
      result: boolean
    }

    try {
      const response = await $b24.actions.v2.call.make<TextFieldEnabledResult>({
        method: 'imbot.v2.Chat.TextField.enabled',
        params: {
          botId: 456,
          dialogId: 'chat5',
          enabled: false,
      ...
      if (!response.isSuccess) {
        ...
        console.info('TextField enabled updated:', result.result)
      }
    } catch (error) {
...
- JS (UMD)
    ...
    <script>
      async function setTextFieldEnabled() {
        try {
      ...
      document.addEventListener('DOMContentLoaded', setTextFieldEnabled)
    </script>
    ```
