...
- JS (TS)
    ...
    try {
      const response = await $b24.actions.v2.call.make<NotifyResult>({
        method: 'imbot.v2.Chat.InputAction.notify',
        params: {
          botId: 456,
          dialogId: 'chat5',
      ...
      if (!response.isSuccess) {
        ...
        console.info('Action indicator sent:', result.result)
      }
    } catch (error) {
...
- JS (UMD)
    ...
    <script>
      async function sendInputActionNotify() {
        try {
      ...
      document.addEventListener('DOMContentLoaded', sendInputActionNotify)
    </script>
    ```
