...
- JS (TS)
    ...
    type UnregisterCommandResult = {
      result: boolean
    }

    try {
      const response = await $b24.actions.v2.call.make<UnregisterCommandResult>({
        method: 'imbot.v2.Command.unregister',
        params: {
          botId: 456,
          commandId: 42,
        },
      ...
      if (!response.isSuccess) {
        ...
        console.info('Command unregistered:', result.result)
      }
    } catch (error) {
...
- JS (UMD)
    ...
    <script>
      ...
      document.addEventListener('DOMContentLoaded', unregisterCommand)
    </script>
    ```
