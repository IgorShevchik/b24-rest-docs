...
- **out-of-tree запись агента — известное ограничение blast-radius.** Проверка работает через
  `git status` и видит только рабочее дерево, поэтому запись по абсолютному пути (`~/.ssh`, `/tmp`)
  она НЕ ловит. Инструмент `Edit` у агента не ограничен по пути файла. Полное закрытие — процессный
  ФС-сэндбоксинг агента (bubblewrap/firejail), либо CLI-allowlist (`--add-dir`/`--allowedDirectories`)
  при его появлении. В тест-харнесе зафиксировано тестом-документацией (тест 7).
