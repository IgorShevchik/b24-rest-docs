...
2. **Никаких устаревших вызовов**: в примере нет `callMethod`, `callListMethod`, `fetchListMethod`.
   Вызов идёт через `actions.v2.call.make` (или `actions.v3.call.make` для rest-v3 / ответа `result.item`).
...
8. **Текст страницы вне табов не изменён** (если доступен git: `git diff -- <PATH>` должен
   затрагивать только блок `{% list tabs %}`). Табы PHP/BX24.js/cURL не трогаем — их русские
   комментарии и пред-существующие `processData()` остаются как есть.
