...
параллельно (`xargs -0 -P`, файлы разные), валидация и запись ledger — серийно (один писатель, без
...
рабочего дерева (`git status`), поэтому запись **вне репозитория** (абсолютный путь — `~/.ssh`,
`/tmp`) она не ловит; полное закрытие требует ФС-сэндбоксинга агента на уровне процесса
(bubblewrap/firejail или `--add-dir`-allowlist) — см. `FOLLOWUPS.md`.
